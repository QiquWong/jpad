...
public class Test09as {
	...
	public static void main(String[] args) {
		...
		// punto A
		airfoilTEPoints.add(new gp_Pnt(1 - flapChordRatio + 0.055 - flapGap,etaInnerSection * wingSemiSpan,airfoilCutZCoords1[0]-0.002));
		// punto B
		airfoilTEPoints.add(new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap,etaInnerSection * wingSemiSpan,airfoilCutZCoords1[0]-0.004));
		...
		// punto C
		airfoilTEPoints.add(new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap, etaInnerSection * wingSemiSpan, airfoilCutZCoords1[1]+0.004));
		// punto D
		airfoilTEPoints.add(new gp_Pnt(1 - flapChordRatio + 0.055 - flapGap, etaInnerSection * wingSemiSpan, airfoilCutZCoords1[1]+0.002));
		...
		// Nuova prova airfoil trailing edge

		Double[] airfoilCutZCoords2 = AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio + 0.05 - flapGap);

		List<gp_Pnt> airfoilTEPoints2 = new ArrayList<>();
		airfoilTEPoints2.add(new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap, etaInnerSection * wingSemiSpan, airfoilCutZCoords2[0]));
		airfoilTEPoints2.add(new gp_Pnt(1 - flapChordRatio - flapGap, 
				etaInnerSection * wingSemiSpan, 
				Math.abs((AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio - flapGap)[1]+AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio - flapGap)[0])) / 2));
		airfoilTEPoints2.add(new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap, etaInnerSection * wingSemiSpan, airfoilCutZCoords2[1]));

		TopoDS_Edge newAirfoilTE2 = TopoDS.ToEdge(((OCCShape) OCCUtils.theFactory.newCurve3DGP(airfoilTEPoints2, false).edge()).getShape());
		// ora prendo due punti sul profilo		
		gp_Pnt pnt1 = new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap - 0.002, etaInnerSection * wingSemiSpan, 
				AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio + 0.05 - flapGap - 0.002)[0]);
		gp_Pnt pnt2 = new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap - 0.002, etaInnerSection * wingSemiSpan, 
				AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio + 0.05 - flapGap - 0.002)[1]);
		
		System.out.println("Coordinate punto 1 : " + pnt1.Coord(1) + " " + pnt1.Coord(2) + " " + pnt1.Coord(3));
		// ora prendo due punti sulla prima curva (newAirfoilTE2)

		Geom_Curve newairfoilTECurve = BRep_Tool.Curve(newAirfoilTE2, new double[1], new double[1]);
		gp_Vec firstTangTE = new gp_Vec();
		gp_Pnt firstPointTE = new gp_Pnt();
		gp_Pnt proj1TE = new gp_Pnt();
		double[] d1 = new double[1];
		long l1 = 0;
		ShapeAnalysis_Curve shapeAn1TE = new ShapeAnalysis_Curve();
		shapeAn1TE.Project(newairfoilTECurve,new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap - 0.002, etaInnerSection * wingSemiSpan, AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio + 0.05 - flapGap - 0.002)[0]), Precision.Confusion(), proj1TE, d1,l1);

		System.out.println("Parametro : " + Arrays.toString(d1));
		newairfoilTECurve.D1(d1[0],firstPointTE, firstTangTE);
		System.out.println("Coordinate punto sulla curva : " + firstPointTE.Coord(1) + "  " + firstPointTE.Coord(2) + "  " + firstPointTE.Coord(3));
		System.out.println("Coordinate punto sul profilo : " + pnt1.Coord(1) + "  " + pnt1.Coord(2) + "  " + pnt1.Coord(3));

		gp_Vec secondTangTE = new gp_Vec();
		gp_Pnt secondPointTE = new gp_Pnt();
		gp_Pnt proj2TE = new gp_Pnt();
		ShapeAnalysis_Curve shapeAn2TE = new ShapeAnalysis_Curve();
		shapeAn2TE.Project(newairfoilTECurve,new gp_Pnt(1 - flapChordRatio + 0.05 - flapGap - 0.002, etaInnerSection * wingSemiSpan, AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio + 0.05 - flapGap - 0.002)[1]), Precision.Confusion(), proj2TE, d1,l1);
		newairfoilTECurve.D1(d1[0],secondPointTE, secondTangTE);

		// ultimi due punti 

		gp_Pnt pnt3 = new gp_Pnt((pnt1.Coord(1) + (1 - flapChordRatio + 0.05 - flapGap))*0.5 + 0.001,etaInnerSection * wingSemiSpan,(pnt1.Coord(3) + firstPointTE.Coord(3))*0.5);
		gp_Pnt pnt4 = new gp_Pnt((pnt2.Coord(1)+ (1 - flapChordRatio + 0.05 - flapGap))*0.5 + 0.001,etaInnerSection * wingSemiSpan,(pnt2.Coord(3) + secondPointTE.Coord(3))*0.5);
		
		// Curva FINALE

		List<gp_Pnt> airfoilTEPointsFinal = new ArrayList<>();
		airfoilTEPointsFinal.add(pnt1);
		airfoilTEPointsFinal.add(pnt3);
		airfoilTEPointsFinal.add(firstPointTE);
		airfoilTEPointsFinal.add(new gp_Pnt(1 - flapChordRatio - flapGap, 
				etaInnerSection * wingSemiSpan, 
				Math.abs((AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio - flapGap)[1]+AircraftUtils.getThicknessAtX(airfoil, 1 - flapChordRatio - flapGap)[0])) / 2));
		airfoilTEPointsFinal.add(secondPointTE);
		airfoilTEPointsFinal.add(pnt4);
		airfoilTEPointsFinal.add(pnt2);
		
		TopoDS_Edge newAirfoilTEFinal = TopoDS.ToEdge(((OCCShape) OCCUtils.theFactory.newCurve3DGP(airfoilTEPointsFinal, false).edge()).getShape());
		...
		exportShapes.add((OCCShape) OCCUtils.theFactory.newShape(newAirfoilTEFinal));
		//exportShapes.add((OCCShape) OCCUtils.theFactory.newShape(newAirfoilTE));
		exportShapes.add((OCCShape) OCCUtils.theFactory.newShape(newAirfoilTE2));

		String fileName = "Test09as.brep";
