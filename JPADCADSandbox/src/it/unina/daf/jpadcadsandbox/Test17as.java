...
public class Test17as {

	public static void main(String[] args) {

		System.out.println("Starting JPADCADSandbox Test17as Fowler Flap procedure");
...
//		double k5 = 0.38;//0.3;
		...
		double k1 = 1; // gap factor
		double k2 = 0.1; //0.3; // airfoil trailing edge factor
		double k3 = 0.06; // 2.3; // flap leading edge factor
		...
		CADEdge middleAirfoilTE = OCCUtils.theFactory.newCurve3D(middleTEPoints,
				false, 
				new double[] {normPntP3.X(), normPntP3.Y(), normPntP3.Z()}, 
				new double[] {tangPntP2.X(), tangPntP2.Y(), 1*tangPntP2.Z()},
				false).edge();
		...
		// Modifica per fowler flap
		
		gp_Pnt P10 = new gp_Pnt(normPntP2.Scaled(-deltaCGap2 * chordLength * 2).Added(new gp_Vec(P2.Coord())).XYZ());
		System.out.println("Point P10 coordinates : " + P10.X() + " " + P10.Y() + " " + P10.Z());
		OCCEdge edgeAirfoil_middle = (OCCEdge) middleAirfoilTE;
		Geom_Curve middleAirfoilTECurve1 = BRep_Tool.Curve(edgeAirfoil_middle.getShape(), new double[1], new double[1]);

//		CADGeomCurve3D middleAirfoilTECurve = OCCUtils.theFactory.newCurve3D(middleTEPoints,
//				false, 
//				new double[] {normPntP3.X(), normPntP3.Y(), normPntP3.Z()}, 
//				new double[] {tangPntP2.X(), tangPntP2.Y(), 1*tangPntP2.Z()},
//				false);
		
		double firstPar = middleAirfoilTECurve1.FirstParameter();
		double lastPar = middleAirfoilTECurve1.LastParameter();
		double parameter = firstPar + (lastPar - firstPar)*0.5;
		gp_Vec tangPntP11 = new gp_Vec();
		gp_Pnt PntP11= new gp_Pnt();
		middleAirfoilTECurve1.D1(parameter, PntP11, tangPntP11);
		tangPntP11.Normalize();
		tangPntP11 = tangPntP11.Reversed();
		
		System.out.println("Point P11 coordinates : " + PntP11.X() + " " + PntP11.Y() + " " + PntP11.Z());
		
		gp_Vec normPntP11 = tangPntP11.Crossed(zyDir).Normalized();
//		normPntP11 = normPntP11*
//		gp_Trsf traslPnt11 = new gp_Trsf();
//		traslPnt11.SetTranslation((normPntP11));
		gp_Pnt P12 = new gp_Pnt(normPntP11.Scaled(deltaCGap2 * chordLength * 4).Added(new gp_Vec(PntP11.Coord())).XYZ());

		
		// Creation of Flap leading edge curve
		
		List<double[]> upperLEPoints1 = new ArrayList<>();
		upperLEPoints1.add(new double[]{P1.Coord(1),P1.Coord(2),P1.Coord(3)});
		upperLEPoints1.add(new double[]{P10.Coord(1),P10.Coord(2),P10.Coord(3)});
		
		CADEdge upperFlapLE1 = OCCUtils.theFactory.newCurve3D(upperLEPoints1,
				false, 
				new double[] {-tangPntP1.X(), -tangPntP1.Y(), -tangPntP1.Z()}, 
				new double[] {-tangPntP2.X(), -tangPntP2.Y(), -1*tangPntP2.Z()},
				false).edge();

		List<double[]> upperLEPoints2 = new ArrayList<>();
		upperLEPoints2.add(new double[]{P10.Coord(1),P10.Coord(2),P10.Coord(3)});
		upperLEPoints2.add(new double[]{P12.Coord(1),P12.Coord(2),P12.Coord(3)});
		
		CADEdge upperFlapLE2 = OCCUtils.theFactory.newCurve3D(upperLEPoints2,
				false, 
				new double[] {-tangPntP2.X(), -tangPntP2.Y(), -1*tangPntP2.Z()}, 
				new double[] {tangPntP11.X(), tangPntP11.Y(), tangPntP11.Z()},
				false).edge();
		
		List<double[]> upperLEPoints3 = new ArrayList<>();
		upperLEPoints3.add(new double[]{P12.Coord(1),P12.Coord(2),P12.Coord(3)});
		upperLEPoints3.add(new double[]{P9.Coord(1),P9.Coord(2),P9.Coord(3)});
		
		CADEdge upperFlapLE3 = OCCUtils.theFactory.newCurve3D(upperLEPoints3,
				false, 
				new double[] {tangPntP11.X(), tangPntP11.Y(), 1*tangPntP11.Z()}, 
		...
		TopoDS_Edge flapUpperLE_Edge1 = TopoDS.ToEdge(((OCCShape) upperFlapLE1).getShape());
		TopoDS_Edge flapUpperLE_Edge2 = TopoDS.ToEdge(((OCCShape) upperFlapLE2).getShape());
		TopoDS_Edge flapUpperLE_Edge3 = TopoDS.ToEdge(((OCCShape) upperFlapLE3).getShape());
		...
		TopoDS_Edge finalFlapLEEdge1 = TopoDS.ToEdge(new BRepBuilderAPI_Transform(flapUpperLE_Edge1, flapTrasl).Shape());
		TopoDS_Edge finalFlapLEEdge2 = TopoDS.ToEdge(new BRepBuilderAPI_Transform(flapUpperLE_Edge2, flapTrasl).Shape());
		TopoDS_Edge finalFlapLEEdge3 = TopoDS.ToEdge(new BRepBuilderAPI_Transform(flapUpperLE_Edge3, flapTrasl).Shape());
		TopoDS_Edge finalFlapLEEdge4 = TopoDS.ToEdge(new BRepBuilderAPI_Transform(flapLowerLE_Edge, flapTrasl).Shape());
		...
		flapWire.Add(finalFlapLEEdge3);
		flapWire.Add(finalFlapLEEdge4);
...
//		exportShapes.add((OCCShape) OCCUtils.theFactory.newShape(firstWingSection));
		...
		String fileName = "Test17as.brep";
		if(OCCUtils.write(fileName,(OCCShape) OCCUtils.theFactory.newShape(flapSecondCut),(OCCShape) OCCUtils.theFactory.newShape(flapFirstCut),(OCCShape) OCCUtils.theFactory.newShape(finalAirfoilCut),lowerAirfoilTE,upperAirfoilTE,middleAirfoilTE,upperFlapLE1,upperFlapLE2,upperFlapLE3,lowerFlapLE))
