...
public class Test26mds {
	...
	public static void main(String[] args) throws IOException {
		...
		if(aeroMap.containsKey(customComponentEnum)) {
			...
			customComponent.getLiftingSurfaceCreator().adjustDimensions(
					originalComponent.getLiftingSurfaceCreator()
							.getAspectRatio()*1.2,
					originalComponent.getLiftingSurfaceCreator().getEquivalentWing().getPanels().get(0)
			...
			customComponent.getLiftingSurfaceCreator().setAirfoilList(originalComponent.getLiftingSurfaceCreator().getAirfoilList());	
		...
		if(aeroMap.containsKey(AeroComponents.WING)) {
			...
			wingMAC = wing.getLiftingSurfaceCreator().getMeanAerodynamicChord().doubleValue(SI.METER);
			wingS = wing.getLiftingSurfaceCreator().getSurfacePlanform().doubleValue(SI.SQUARE_METRE);
			wingSpan = wing.getLiftingSurfaceCreator().getSpan().doubleValue(SI.METER);
			momentPoleXCoord = wing.getXApexConstructionAxes().doubleValue(SI.METER) + 
	                  		   wing.getLiftingSurfaceCreator().getMeanAerodynamicChordLeadingEdgeX().doubleValue(SI.METER) + 
	                           wing.getLiftingSurfaceCreator().getMeanAerodynamicChord().doubleValue(SI.METER)*0.25;
		} else {
