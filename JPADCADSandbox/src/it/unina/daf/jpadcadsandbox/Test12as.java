...
import opencascade.BRepAlgoAPI_Algo;
...
import opencascade.BRepExtrema_DistanceSS;
...
public class Test12as {

	public static void main(String[] args) {

		System.out.println("Starting JPADCADSandbox Test12as");
				...
				gp_Vec zyDir = liftingSurface.getType().equals(ComponentEnum.VERTICAL_TAIL) ? // auxiliary vectors, useful for wire construction
						...
						gp_Vec yzDir = liftingSurface.getType().equals(ComponentEnum.VERTICAL_TAIL) ? 
								...
								if(liftingSurface.getType().equals(ComponentEnum.VERTICAL_TAIL)) {
									...
									if( i == 0 ) {
										flapTrasl.SetTranslation(new gp_Pnt(0, 0, etaInnerFlap*wingSemiSpan), new gp_Pnt(0, 0, etaInnerFlap * wingSemiSpan + flapLateralGap));
									}
									if( i == 1 )	{

										flapTrasl.SetTranslation(new gp_Pnt(0, 0, etaOuterFlap*wingSemiSpan), new gp_Pnt(0, 0, etaOuterFlap * wingSemiSpan - flapLateralGap));
								...
								flapCutWire_Flap.add(OCCUtils.theFactory.newWireFromAdjacentEdges((CADEdge) OCCUtils.theFactory.newShape(finalFlap1),(CADEdge) OCCUtils.theFactory.newShape(finalFlapUpperLE), (CADEdge) OCCUtils.theFactory.newShape(finalFlapLowerLE),(CADEdge) OCCUtils.theFactory.newShape(finalFlap2), (CADEdge) OCCUtils.theFactory.newShape(finalFlapTE)));
								...
								flapCutWire.add(OCCUtils.theFactory.newWireFromAdjacentEdges((CADEdge) OCCUtils.theFactory.newShape(finalAirfoilCut),(CADEdge) OCCUtils.theFactory.newShape(airfoilUpperTE_Edge), (CADEdge) OCCUtils.theFactory.newShape(airfoilLowerTE_Edge), (CADEdge) OCCUtils.theFactory.newShape(airfoilMiddleUpperTE_Edge), (CADEdge) OCCUtils.theFactory.newShape(airfoilMiddleLowerTE_Edge)));
								...
								exportInnerCutFlap.add((OCCShape) OCCUtils.theFactory.newShape(airfoilMiddleUpperTE_Edge));
								exportInnerCutFlap.add((OCCShape) OCCUtils.theFactory.newShape(airfoilMiddleLowerTE_Edge));
		...
		int numDiv = 5;
		boolean interference = checkInterferenceSym(liftingSurface, minDeflection, maxDeflection, 
				(OCCShape) OCCUtils.theFactory.newShape(finalWingShape), Flapsolids.get(0), numDiv);
		
		// Prova rotazione
		OCCShape rotatedFlap = symFlapRotation(liftingSurface, 0.523, 
				(OCCShape) OCCUtils.theFactory.newShape(finalWingShape), Flapsolids.get(0));
		
		// Set flap rotation
		CADGeomCurve3D innerChord =  getChordSegmentAtYActual(Flapsolids.get(0).boundingBox()[1], liftingSurface);
		double innerChordLength = innerChord.length();
		CADGeomCurve3D outerChord =  getChordSegmentAtYActual(Flapsolids.get(0).boundingBox()[4], liftingSurface);
		...
		innerHingePos.SetX((innerChordLength * innerHingePntx) + liftingSurface.getXLEAtYActual(Flapsolids.get(0).boundingBox()[1]).doubleValue(SI.METER) + liftingSurface.getXApexConstructionAxes().doubleValue(SI.METER));
		innerHingePos.SetY(Flapsolids.get(0).boundingBox()[1]);
		innerHingePos.SetZ((innerChordLength * innerHingePntz) + liftingSurface.getZApexConstructionAxes().doubleValue(SI.METER));

		outerHingePos.SetX((outerChordLength * outerHingePntx) + liftingSurface.getXLEAtYActual(Flapsolids.get(0).boundingBox()[4]).doubleValue(SI.METER) + liftingSurface.getXApexConstructionAxes().doubleValue(SI.METER));
		outerHingePos.SetY(Flapsolids.get(0).boundingBox()[4]);
...
//		BRepBuilderAPI_Transform transf = new BRepBuilderAPI_Transform(Flapsolids.get(0).getShape(), rotation);
//		TopoDS_Shape rotatedFlap = transf.Shape();
//		TopoDS_Shape intersectedSolid = new BRepAlgoAPI_Cut(finalWingShape,rotatedFlap).Shape();
//		OCCShape intersectedShape = (OCCShape) OCCUtils.theFactory.newShape(intersectedSolid);
//		System.out.println(" Report on shape : " +OCCUtils.reportOnShape(intersectedSolid));
//		System.out.println("Is null the intersection " + intersectedSolid.IsNull());

		
		
//		TopoDS_Solid auxCutWing = performBooleanCutOperation(
//				new Tuple2<TopoDS_Solid, Boolean>(TopoDS.ToSolid(finalWingShape), true), 
//				new Tuple2<TopoDS_Solid, Boolean>(TopoDS.ToSolid(rotatedFlap), true)
//				).get(0);

//		(OCCShape) OCCUtils.theFactory.newShape(finalWingShape)
		
		//		innerHingePos.SetY(flap.boundingBox()[1]);
//		innerHingePos.SetZ((innerChordLength * innerHingePntz) + liftingSurface.getZApexConstructionAxes().doubleValue(SI.METER));

//		
//		TopoDS_Shape intersectedSolid = new BRepAlgoAPI_Common(Flapsolids.get(0).getShape(),finalWingShape).Shape();
////		Boolean result = intersectedSolid.IsNull();
//
//		System.out.println("Intersection result : " + intersectedSolid.IsNull());
		// Mirroring
		...
		String fileName = "Test12as.brep";
		if(OCCUtils.write(fileName, airfoils))
			System.out.println("========== [main] Output written on file: " + fileName);
		if(!typeLS.equals(ComponentEnum.VERTICAL_TAIL)) {  
//			OCCUtils.write(fileName, Flapsolids,(OCCShape) OCCUtils.theFactory.newShape(finalWingShape),leftSideFlaps,leftSideWing);
			OCCUtils.write(fileName, rotatedFlap,(OCCShape) OCCUtils.theFactory.newShape(finalWingShape),leftSideFlaps,leftSideWing);
//			OCCUtils.write(fileName, (OCCShape) OCCUtils.theFactory.newShape(intersectedSolid));
//			OCCUtils.write(fileName, (OCCShape) OCCUtils.theFactory.newShape(auxCutWing));
	...
	public static boolean checkInterferenceSym(LiftingSurface liftingSurface, double minDeflection, double maxDeflection, 
			OCCShape wing, OCCShape flap, int numDiv) {
