...
class MyArgumentsAnalysis {
	...
	@Option(name = "-ia", aliases = { "--input-analyses" }, required = true,
			usage = "aerodynamic analyses input file")
	private File _inputFileAerodynamicAnalyses;
	...
	public File getInputFileAerodynamicAnalyses() {
		return _inputFileAerodynamicAnalyses;
	}
...
public class ACAerodynamicAndStabilityManager_v2_Test extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Aircraft aircraft = ACAerodynamicAndStabilityManager_v2_Test.theAircraft;
		if (aircraft == null) {
	...
	public static void main(String[] args) throws InvalidFormatException, HDF5LibraryException {
		...
		ACAerodynamicAndStabilityManager_v2_Test.theCmdLineParser = new CmdLineParser(va);

		// populate the wing static object in the class-> start ...)
		try {
		// before launching the JavaFX application thread (launch -
			ACAerodynamicAndStabilityManager_v2_Test.theCmdLineParser.parseArgument(args);
			
			String pathToXML = va.getInputFile().getAbsolutePath();
			System.out.println("AIRCRAFT INPUT ===> " + pathToXML);

			String pathToAnalysesXML = va.getInputFileAerodynamicAnalyses().getAbsolutePath();
			System.out.println("AERODYNAMIC ANALYSES INPUT ===> " + pathToAnalysesXML);
			...
			System.setOut(filterStream);
			ACAerodynamicAndStabilityManager_v2 aeroManager = ACAerodynamicAndStabilityManager_v2.importFromXML(
					pathToAnalysesXML,
					theAircraft, 
			...
			ACAerodynamicAndStabilityManager_v2_Test.theCmdLineParser.printUsage(System.err);
			System.err.println();
