package sandbox2.mr.Stability2;
...
public class StabilityExecutableManager {
	...
	// 3D CURVES FROM INPUT -------------------
	private List<Amount<Angle>> alphaForClCurve = new ArrayList<>();
	private List<Double> clInputCurve = new ArrayList<>();
	private List<Amount<Angle>> alphaForCmCurve = new ArrayList<>();
	private List<Double> momentInputCurve = new ArrayList<>();
 ...
 cdDistributionFuselageFinal	= MyArrayUtils.convertDoubleArrayToListDouble(
	...
	public void initializeCalculators(){	
		...
		theNasaBlackwellCalculatorMachActualWing = new NasaBlackwell(
				...
				_wingAlphaZeroLiftDistribution,
		...
		if( _theCondition == ConditionEnum.TAKE_OFF || _theCondition == ConditionEnum.LANDING){
			theNasaBlackwellCalculatorMachActualWingCLEAN = new NasaBlackwell(
					...
					_wingAlphaZeroLiftDistributionCLEAN,
					_wingVortexSemiSpanToSemiSpanRatio,
					0.0,
					this._machCurrent,
					this.getAltitude().doubleValue(SI.METER)
...
//_downwashAngleVariableSlingerlandOld= theStabilityCalculator.getDownwashAngle();
	...
	public String printAllResults(){
		...
		.append("\tWing\n")
		.append("\t-------------------------------------\n")
		.append("\t\talpha zero lift = " + _wingAlphaZeroLiftCONDITION+ "\n" )
		.append("\t\tCL zero = " + _wingcLZeroCONDITION+ "\n")
		.append("\t\tCL alpha = " + _wingclAlphaCONDITION+ "\n")
		.append("\t\tCL alpha M=0 = " + cLAlphaMachZero+ "\n")
		.append("\t\tCL star = " + _wingcLStarCONDITION+ "\n")
		.append("\t\tAlpha star = " + _wingalphaStarCONDITION+ "\n")
	...
	public void plot( String folderPathName, String folderPathNameDistribution) throws InstantiationException, IllegalAccessException{
		...
		if(_plotList.contains(AerodynamicAndStabilityPlotEnum.WING_CL_CURVE_CLEAN)) {
			...
			MyChartToFileUtils.plot(
					xList, 
					yList, 
					"Wing Lift Coefficient 3D curve", 
					"alpha_w", "CL", 
					null, null,
					null, null,
					"deg", "",
					...
					legend,
					folderPathName,
					"Wing Lift Coefficient 3D curve", true);
...
//		legend = new ArrayList<>();
	...
	public void calculateWingLiftCharacteristics(){
		...
		Double [] clArray = new Double [_numberOfAlphasBody];
		
		_wingliftCoefficient3DCurveCONDITION= MyMathUtils.getInterpolatedValue1DLinear(
				MyArrayUtils.convertListOfAmountTodoubleArray(alphaForClCurve), 
				MyArrayUtils.convertToDoublePrimitive(clInputCurve),
				MyArrayUtils.convertListOfAmountTodoubleArray(_alphasWing));
		

			//clAlpha Variable
			this._wingclAlphaArray = LiftCalc.calculateCLAlphaArray(_wingliftCoefficient3DCurveCONDITION, _alphasWing);
		...
		for (int i=0; i<this._anglesOfElevatorDeflection.size(); i++){
		 ...
		 this._deltaEAnglesArray = MyArrayUtils.convertDoubleArrayToListOfAmount(
				MyArrayUtils.convertFromDoubleToPrimitive(
						MyArrayUtils.linspace(-42.84, 5, 10)), NonSI.DEGREE_ANGLE);
...
//	}
	...
	public void calculateWingMomentCharacterstics(){
		...
		Double [] momentArray = new Double [_numberOfAlphasBody];
		
		momentArray = MyMathUtils.getInterpolatedValue1DLinear(
				MyArrayUtils.convertListOfAmountTodoubleArray(alphaForCmCurve), 
				MyArrayUtils.convertToDoublePrimitive(momentInputCurve),
				MyArrayUtils.convertListOfAmountTodoubleArray(_alphasWing));
		
		for (int i=0; i<_numberOfAlphasBody; i++){
			_wingMomentCoefficientFinal.add(i,
					momentArray[i]);
		}
	}
	public void calculateHtailMomentCharacterstics() throws InstantiationException, IllegalAccessException{
	   ...
	   hTailMomentumPoleYH = Amount.valueOf( 
		...
		// respect TO FINAL POLE
			...
			for (int ii=0; ii<_numberOfAlphasBody; ii++){
			_hTailMomentCoefficientFinal.add(ii, 0.0);
			}
...
//				
	...
	public void calculateHTailEquilibriumLiftCoefficient(){
		...
		for (int i=0; i<_numberOfAlphasBody; i++){
			...
			_hTailEquilibriumLiftCoefficient.add(i,
					(-_wingMomentCoefficientPendular.get(i)-_fuselageMomentCoefficient.get(i)-_fuselageMomentCoefficientdueToDrag.get(i)-_landingGearMomentDueToDrag.get(i))*
	...
	public void calculateDeltaeEquilibrium() throws InstantiationException, IllegalAccessException{
		...
		double tauActual=0, clZero, clMax, clStar;
...
//			}
	...
	public List<Amount<Angle>> getAlphaForClCurve() {
		return alphaForClCurve;
	}

	public List<Double> getClInputCurve() {
		return clInputCurve;
	}


	public List<Double> getMomentInputCurve() {
		return momentInputCurve;
	}

	public void setAlphaForClCurve(List<Amount<Angle>> alphaForClCurve) {
		this.alphaForClCurve = alphaForClCurve;
	}

	public void setClInputCurve(List<Double> clInputCurve) {
		this.clInputCurve = clInputCurve;
	}


	public void setMomentInputCurve(List<Double> momentInputCurve) {
		this.momentInputCurve = momentInputCurve;
	}

	public List<Amount<Angle>> getAlphaForCmCurve() {
		return alphaForCmCurve;
	}

	public void setAlphaForCmCurve(List<Amount<Angle>> alphaForCmCurve) {
		this.alphaForCmCurve = alphaForCmCurve;
	}

	public MethodEnum getTauInput() {
		return _tauInput;
	}

	public void setTauInput(MethodEnum _tauInput) {
		this._tauInput = _tauInput;
	}

	public Double get_tauIndex() {
		return _tauIndex;
	}

	public void set_tauIndex(Double _tauIndex) {
		this._tauIndex = _tauIndex;
	}
