...
%
\begin{equation}
S_{w,f}=\dfrac{1}{2}\cdot\left(c\left(\eta_{\text{in}}\right)+c\left(\eta_{\text{out}}\right)\right)\cdot\left(\eta_{\text{out}}-\eta_{\text{in}}\right)\cdot\dfrac{b}{2}
\label{eqn:Swf}
\end{equation}
%
\begin{figure}[!b]
...
%
\noindent
In the (\ref{eqn:Swf}) the quantity $c\left(\eta\right)$ is the value of the wing chord at a specific station, while $\eta_i$ and $\eta_o$ are, respectively, the non-dimensional inboard location and the outboard location of the flap. If the flaps do not extend continuously along the trailing edge then the affected area for each may be calculated independently and added together.
