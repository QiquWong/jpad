...
\bigskip
\noindent
The most important method of the class is~\lstinline[language=Java]!calculateTakeOffDistanceODE! which is in charge of the resolution of the ODE set presented in (\ref{eq:Take:Off:System:Dynamics:A}). This method accepts as input two parameters which are used to determine, firstly, if an engine failure has occurred during the take-off run, and then if the take-off run has to be aborted; these are:
%
\begin{itemize}
...
\end{itemize}
%
After performig a check upon these two variables, the method knows which case it has to study and proceed with the calculation; in particular it creates the integrator object of the class \lstinline[language=Java]!FirstOrderIntegrator!
