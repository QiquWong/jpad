...
\newdualentry{TiKZ} % label
  {TikZ}            % abbreviation
  {TikZ}  % long form
  {Is a set of higher-level macros that use PGF} % description
...
\newglossaryentry{Table}{%
   name=Table,
   description={%
      Is a collection that associates an ordered pair of keys, called a row key and a column key, with a single value. A table may be sparse, with only a small fraction of row key / column key pairs possessing a corresponding value}
}
