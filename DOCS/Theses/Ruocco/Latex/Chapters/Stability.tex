...
\subsection{Fuselage}
The moment coefficient of the fuselage, for what, since it is a pure couple, is not necessary specify the pole, assuming the linearity, is represented by the following equation in function of the angle of attack.
...
Each term in the equation \ref{eq:fus} can be calculated by the Multhopp method.  Multhopp has developed a method of calculating the local lift and pitching moment on wings of any plan-form in subsonic steady flow.  The bodymust be divided for all its length, in equal parts, regardless of the wing position.
