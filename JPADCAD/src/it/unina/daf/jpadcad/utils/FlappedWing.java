...
public class FlappedWing {
	...
	Boolean exportShells;
	Boolean exportSolids;
	Boolean exportSupportShapes;
	Boolean isFlapped;
	Boolean isSlatted;
	double yInnerPct, yOuterPct;
	double innerChordRatio, outerChordRatio;
	double innerChordRatioSlat, outerChordRatioSlat;
	double lateralGap;
	FlapType flapType;
	...
	public static Map<SolidType, List<OCCShape>> getFlappedWingCAD(LiftingSurface liftingSurface, Boolean exportWing, Boolean exportFlap, 
		...
		double lateralGap = 0.025; // liftingSurface.getLateralGap();
		boolean horn = false;  // liftingSurface.getHorn();
		...
		System.out.println("Wing section breakpoints : ");
		...
		Map<ShellType, List<OCCShape>> shellMap = new HashMap<>();
		shellMap.put(ShellType.WING_CLEAN, new ArrayList<OCCShape>());
		shellMap.put(ShellType.WING_CUT, new ArrayList<OCCShape>());
		shellMap.put(ShellType.FLAP, new ArrayList<OCCShape>());
		shellMap.put(ShellType.SLAT, new ArrayList<OCCShape>());		
		...
		for( int i = 0; i < numSections ; i++ ) {
			...
			double innerChordRatio = 0.0;
			double outerChordRatio = 0.0;
			double innerChordRatioSlat = 0.0;
			double outerChordRatioSlat = 0.0;
			...
			System.out.println("Outer section : " + yOuter);
				...
				System.out.println("Is Flapped? " + isFlapped + " "  + "Is Slatted? "+ isSlatted);
				if(isFlapped) {
					System.out.println("FLAP inner chord ratio  : " + innerChordRatio);
					System.out.println("FLAP outer chord ratio : " + outerChordRatio);
				}
				if(isSlatted) {
					System.out.println("SLAT inner chord ratio  : " + innerChordRatioSlat);
					System.out.println("SLAT outer chord ratio : " + outerChordRatioSlat);
				}
		...
		if(exportWing) {
			...
			sectionToSew.addAll(shellMap.get(ShellType.WING_CLEAN));
			sectionToSew.addAll(shellMap.get(ShellType.WING_CUT));
			BRepBuilderAPI_Sewing sewingWingSection = new BRepBuilderAPI_Sewing();
			sectionToSew.forEach(s -> sewingWingSection.Add(s.getShape()));
			sewingWingSection.Perform();
			TopoDS_Shape sewedWing = sewingWingSection.SewedShape();
			TopoDS_Shape sewedWingFinal = null;
			...
			if(!horn) {
				...
				sewingTip.Add(makeTip(liftingSurface));
				sewingTip.Add(sewedWing);
				sewingTip.Perform();
				TopoDS_Shape sewedWingAndTip = sewingTip.SewedShape();

				ShapeAnalysis_FreeBounds shapeAnalayzer = new ShapeAnalysis_FreeBounds(sewedWingAndTip);
				...
				BRepBuilderAPI_Sewing sewingFreeBounds = new BRepBuilderAPI_Sewing();
				sewingFreeBounds.Init();
				sewingFreeBounds.Add(sewedWingAndTip);
				faces.forEach(f -> sewingFreeBounds.Add(((OCCShape) f).getShape()));	
				sewingFreeBounds.Perform();
				sewedWingFinal = sewingFreeBounds.SewedShape();

			}

			else {

				ShapeAnalysis_FreeBounds shapeAnalayzer = new ShapeAnalysis_FreeBounds(sewedWing);
				TopExp_Explorer wireExplorer = new TopExp_Explorer(shapeAnalayzer.GetClosedWires(), TopAbs_ShapeEnum.TopAbs_WIRE);
				List<CADWire> wireList = new ArrayList<>();
				while( wireExplorer.More() == 1 )	{
				...
				BRepBuilderAPI_Sewing sewingFreeBounds = new BRepBuilderAPI_Sewing();
				sewingFreeBounds.Init();
				sewingFreeBounds.Add(sewedWing);
				faces.forEach(f -> sewingFreeBounds.Add(((OCCShape) f).getShape()));	
				sewingFreeBounds.Perform();
				sewedWingFinal = sewingFreeBounds.SewedShape();
			...
			makeSolid.Add(TopoDS.ToShell(sewedWingFinal));
			makeSolid.Build();
			TopoDS_Shape solidShapeWing = makeSolid.Shape();

			solidsMap.get(SolidType.WING).add((OCCShape) OCCUtils.theFactory.newShape(solidShapeWing));
			
		}

		if(exportFlap) {
			// Sewing of Flap sections
			System.out.println("Sewing flap and slat shells");
			List<OCCShape> sewedFlapsAndSlat = new ArrayList<>();
			for( int i = 0; i < shellMap.get(ShellType.FLAP).size(); i++) {

				if(( horn == false ) || ( horn == true && i < shellMap.get(ShellType.FLAP).size() - 2 )) {
					ShapeAnalysis_FreeBounds shapeAnalayzer = new ShapeAnalysis_FreeBounds(shellMap.get(ShellType.FLAP).get(i).getShape());
					TopExp_Explorer wireExplorer = new TopExp_Explorer(shapeAnalayzer.GetClosedWires(), TopAbs_ShapeEnum.TopAbs_WIRE);
					List<CADWire> wireList = new ArrayList<>();
					while( wireExplorer.More() == 1)	{
					...
					BRepBuilderAPI_Sewing sewingFlap = new BRepBuilderAPI_Sewing();
					sewingFlap.Add(shellMap.get(ShellType.FLAP).get(i).getShape());
					faces.forEach(f -> sewingFlap.Add(((OCCShape) f).getShape()));	
					sewingFlap.Perform();
					TopoDS_Shape sewedFlap = sewingFlap.SewedShape();

					BRepBuilderAPI_MakeSolid makeSolidFlap = new BRepBuilderAPI_MakeSolid();
					makeSolidFlap.Add(TopoDS.ToShell(sewedFlap));
					makeSolidFlap.Build();
					TopoDS_Shape solidShapeFlap = makeSolidFlap.Shape();
					sewedFlapsAndSlat.add((OCCShape) OCCUtils.theFactory.newShape(solidShapeFlap));

					solidsMap.get(SolidType.FLAP).add((OCCShape) OCCUtils.theFactory.newShape(solidShapeFlap));
					...
					// First sewing
					System.out.println("First Sewing");
					BRepBuilderAPI_Sewing sewingHorn = new BRepBuilderAPI_Sewing();
					sewingHorn.Add(shellMap.get(ShellType.FLAP).get(i).getShape());
					sewingHorn.Add(shellMap.get(ShellType.FLAP).get(i+1).getShape());
					sewingHorn.Perform();
					TopoDS_Shape sewedHorn = sewingHorn.SewedShape();

					// Second sewing
					System.out.println("Second Sewing");
					BRepBuilderAPI_Sewing sewingTip = new BRepBuilderAPI_Sewing();
					sewingTip.Init();		
					sewingTip.SetTolerance(1E-3);
					sewingTip.Add(sewedHorn);
					sewingTip.Add(makeTip(liftingSurface));
					sewingTip.Perform();
					TopoDS_Shape sewedHornAndTip = sewingTip.SewedShape();

					ShapeAnalysis_FreeBounds shapeAnalayzer = new ShapeAnalysis_FreeBounds(sewedHornAndTip);
					TopExp_Explorer wireExplorer = new TopExp_Explorer(shapeAnalayzer.GetClosedWires(), TopAbs_ShapeEnum.TopAbs_WIRE);
					List<CADWire> wireList = new ArrayList<>();
					while( wireExplorer.More() == 1)	{
					...
					// Third sewing
					System.out.println("Third Sewing");
					BRepBuilderAPI_Sewing sewingFreeBounds = new BRepBuilderAPI_Sewing();
					sewingFreeBounds.Init();
					sewingFreeBounds.Add(sewedHornAndTip);
					faces.forEach(f -> sewingFreeBounds.Add(((OCCShape) f).getShape()));	
					sewingFreeBounds.Perform();
					TopoDS_Shape sewedFlap = sewingFreeBounds.SewedShape();
					System.out.println("Making solid flap");
					BRepBuilderAPI_MakeSolid makeSolidFlap = new BRepBuilderAPI_MakeSolid();
					makeSolidFlap.Add(TopoDS.ToShell(sewedFlap));
					makeSolidFlap.Build();
					TopoDS_Shape solidShapeFlap = makeSolidFlap.Shape();
		...
		if(exportSlat) {
			// Sewing of Slat shells
			for( int i = 0; i < shellMap.get(ShellType.SLAT).size(); i++) {

				ShapeAnalysis_FreeBounds shapeAnalayzer = new ShapeAnalysis_FreeBounds(shellMap.get(ShellType.SLAT).get(i).getShape());
				TopExp_Explorer wireExplorer = new TopExp_Explorer(shapeAnalayzer.GetClosedWires(), TopAbs_ShapeEnum.TopAbs_WIRE);
				List<CADWire> wireList = new ArrayList<>();
				while( wireExplorer.More() == 1)	{
				...
				BRepBuilderAPI_Sewing sewingSlat = new BRepBuilderAPI_Sewing();
				sewingSlat.Add(shellMap.get(ShellType.SLAT).get(i).getShape());
				faces.forEach(f -> sewingSlat.Add(((OCCShape) f).getShape()));	
				sewingSlat.Perform();
				TopoDS_Shape sewedSlat = sewingSlat.SewedShape();

				BRepBuilderAPI_MakeSolid makeSolidSlat = new BRepBuilderAPI_MakeSolid();
				makeSolidSlat.Add(TopoDS.ToShell(sewedSlat));
				makeSolidSlat.Build();
				TopoDS_Shape solidShapeFlap = makeSolidSlat.Shape();
				solidsMap.get(SolidType.SLAT).add((OCCShape) OCCUtils.theFactory.newShape(solidShapeFlap));
	...
	public static void getShell(Map<ShellType, List<OCCShape>> shellMap, LiftingSurface liftingSurface, double yInnerPct, double yOuterPct, Boolean isFlapped, FlapType flapType, 
		...
		if(!isFlapped && doMakeHorn == false) {
			...
			makeShellClean(shellMap, airfoilsClean, liftingSurface, yInnerPct, yOuterPct, doMakeHorn);

		}

		if(!isFlapped && doMakeHorn == true) {

			doMakeHorn(shellMap, liftingSurface, airfoilsClean);
		...
		if (isFlapped) {
			...
			makeShellFlap(shellMap, airfoilsCutAndFlap);
	...
	public static void makeShellClean(Map<ShellType, List<OCCShape>> shellMap, List<CADEdge> airfoilsClean, 
		...
		double[] crvR2TE = selectedCurves.get(selectedCurves.size()-1).getRange();
		CADFace face1TE = OCCUtils.theFactory.newFacePlanar(
				...
				selectedCurves.get(selectedCurves.size()-1).value(crvR2TE[0])
				);

		CADFace face2TE = OCCUtils.theFactory.newFacePlanar(
				selectedCurves.get(selectedCurves.size()-1).value(crvR2TE[0]),
				selectedCurves.get(selectedCurves.size()-1).value(crvR2TE[1]),
				selectedCurves.get(0).value(crvR1TE[1])
				);
		...
		sewMakerWing.Add(((OCCShape)newShellTE).getShape());
		sewMakerWing.Perform();
		TopoDS_Shape sewedSection = sewMakerWing.SewedShape();

		shellMap.get(ShellType.WING_CLEAN).add((OCCShape) OCCUtils.theFactory.newShape(sewedSection));
	...
	public static void makeShellCut(Map<ShellType, List<OCCShape>> shellMap, List<List<CADWire>> airfoilsCutAndFlap, 
		...
		if (Math.abs(Math.abs(yOuterPct * liftingSurface.getSemiSpan().doubleValue(SI.METER)) - 
				Math.abs(yInnerPct * liftingSurface.getSemiSpan().doubleValue(SI.METER))) > 1.0) {
			selectedWires.addAll(airfoilsCutAndFlap.get(0));
			System.out.println("Using support airfoils");
		}

		OCCShape Shape = (OCCShape) OCCUtils.theFactory.newShell(selectedWires);
	...
	public static  List<List<CADWire>> makeAirfoilsSlat(LiftingSurface liftingSurface, List<CADEdge> airfoilsClean, List<List<CADWire>> airfoilsCutAndFlap, 
		...
		if (isFlapped) {
			...
			for( int i = 0; i < 2; i++ ) {
				...
				double chordLength = chord.length();
				OCCEdge chordEdge = (OCCEdge) chord.edge();
				List<CADEdge> airfoilsCutEdges = new ArrayList();
				if (i == 0) {
					airfoilsCutEdges = airfoilsCutAndFlap.get(0).get(i).edges();
				}
				else {
					airfoilsCutEdges = airfoilsCutAndFlap.get(0).get(airfoilsCutAndFlap.get(0).size()-1).edges();
				}
			...
			for( int i = 0; i < 2; i++ ) {
				...
				CADGeomCurve3D chord = null;
				OCCEdge airfoilEdge = null;
				if(i == 0) {
					chord = getChordSegmentAtYActual(yInnerPct * liftingSurface.getSemiSpan().doubleValue(SI.METER), liftingSurface);
					airfoilEdge = (OCCEdge) airfoilsClean.get(i);
				}
				else {
					chord = getChordSegmentAtYActual(yOuterPct * liftingSurface.getSemiSpan().doubleValue(SI.METER), liftingSurface);
					airfoilEdge = (OCCEdge) airfoilsClean.get(airfoilsClean.size()-1);
				}
	...
	public static void doMakeHorn(Map<ShellType, List<OCCShape>> shellMap, LiftingSurface liftingSurface, List<CADEdge> airfoilsClean) {
		...
		List<CADGeomCurve3D> airfoilCurve = new ArrayList<CADGeomCurve3D>();
		airfoilCurve.add(OCCUtils.theFactory.newCurve3D(airfoilsClean.get(0)));
		airfoilCurve.add(OCCUtils.theFactory.newCurve3D(airfoilsClean.get(1)));

		OCCShape Shape = OCCUtils.makePatchThruCurveSections(airfoilCurve);

		double[] crvR1TE = airfoilCurve.get(0).getRange();
		double[] crvR2TE = airfoilCurve.get(airfoilCurve.size()-1).getRange();
		CADFace face1TE = OCCUtils.theFactory.newFacePlanar(
				airfoilCurve.get(0).value(crvR1TE[0]),
				airfoilCurve.get(0).value(crvR1TE[1]),
				airfoilCurve.get(1).value(crvR2TE[0])
				);

		CADFace face2TE = OCCUtils.theFactory.newFacePlanar(
				airfoilCurve.get(1).value(crvR2TE[0]),
				airfoilCurve.get(1).value(crvR2TE[1]),
				airfoilCurve.get(0).value(crvR1TE[1])
				);
		CADShell newShellTE = OCCUtils.theFactory.newShellFromAdjacentFaces(face1TE, face2TE);
	...
	public static OCCShape symFlapRotation(LiftingSurface liftingSurface, double deflection, OCCShape flap, double innerHingePntx, double innerHingePntz,
		...
		System.out.println("Hinge inner coordinates : " + innerHingePos.X() + " " + innerHingePos.Y() + " " + innerHingePos.Z());
		System.out.println("Hinge outer coordinates : " + outerHingePos.X() + " " + outerHingePos.Y() + " " + outerHingePos.Z());
	...
	public static OCCShape nonSymRotation(LiftingSurface liftingSurface, double deflection, OCCShape wing, OCCShape flap,
		...
		System.out.println("MiddleRefPnt coordinates : " + middleRefPnt.X() + " " + middleRefPnt.Y() + " " + middleRefPnt.Z());
		...
		// ----------------------------------------------------------	
		gp_Trsf rotation = new gp_Trsf();
		rotation.SetRotation(refAxis,deflection);
		...
		System.out.println("InnerRefPnt extr : " + innerRefPntExtr.X() + " " + innerRefPntExtr.Y() + " " + innerRefPntExtr.Z());
		System.out.println("MiddleRefPnt extr : " + middleRefPntExtr.X() + " " + middleRefPntExtr.Y() + " " + middleRefPntExtr.Z());
		System.out.println("OuterRefPnt extr : " + outerRefPntExtr.X() + " " + outerRefPntExtr.Y() + " " + outerRefPntExtr.Z());
		...
		System.out.println("Roll rotation angle : " + gamma);
		...
		System.out.println("Yaw rotation angle : " + beta);

		// ----------------------------------------------------------
		// Flap movement
		// ----------------------------------------------------------

		BRepBuilderAPI_Transform transf0 = new BRepBuilderAPI_Transform(flap.getShape(), rotation);
	...
	public static void checkInterferenceSym(LiftingSurface liftingSurface, double minDeflection, double maxDeflection, 
		...
		for(int i = 0; i < deflectionVec.length; i++) {
			...
			TopoDS_Shape intersectedSolid = new BRepAlgoAPI_Cut(fixedWing,rotatedFlap).Shape();
