...
public class OCCFXFuselageSection {
	...
	private List<org.fxyz3d.geometry.Point3D> _knots = new ArrayList<org.fxyz3d.geometry.Point3D>();
	// the interpolating bezier
	private InterpolateBezier _interpolate;

	private Double _x;
	private int _idxX = 0;
	public OCCFXFuselageSection(List<org.fxyz3d.geometry.Point3D> pts) { 
		_knots = pts;
	...
	public OCCFXFuselageSection(Fuselage fuselage, int idx) {
		if (fuselage != null) {
			...
			_knots = section.getSectionLeftPoints() // Gives PVectors
					.stream()
					.map(pVector -> { // <===== LAMBDA
						org.fxyz3d.geometry.Point3D p3D = new org.fxyz3d.geometry.Point3D(0f, 0f, 0f);
						p3D.x = _x.floatValue();
						p3D.y = pVector.x;
						p3D.z = pVector.y;
						return p3D;
					}) // want org.fxyz3d.geometry.Point3D
	...
	public List<org.fxyz3d.geometry.Point3D> getKnots() {
		return _knots;
	}
}
