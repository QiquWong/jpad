...
//			System.out.println("OCCGeomCurve3D - range: " + Arrays.toString(range));		
	...
	public OCCGeomCurve3D(List<double[]> pointList, boolean isPeriodic, double[] initialTangent, double[] finalTangent, boolean doScale) {
		// curve
		int nPoints = pointList.size();
		if (nPoints > 0) {			
			TColgp_HArray1OfPnt points = new TColgp_HArray1OfPnt(1, nPoints);
			...
			GeomAPI_Interpolate anInterpolate = 
					new GeomAPI_Interpolate(points, longIsPeriodic, Precision.Confusion());
			// enforce tangency constraints at both end of the curve
			long longDoScale = 0;
			if (doScale)
				longDoScale = 1;
			anInterpolate.Load(
					new gp_Vec(initialTangent[0], initialTangent[1], initialTangent[2]), 
					new gp_Vec(finalTangent[0], finalTangent[1], finalTangent[2]), 
					longDoScale);
