...
public class LiftingSurface2Panels extends AeroComponent implements ILiftingSurface2Panels {
	 ...
	 * 
	 * N+ = Necessary
	 * N+*= Transition to general multipanel planform
	 */
	...
	Double _aspectRatio = null; // N+
	Double _taperRatioEquivalent = null; // N+
	Double _taperRatioActual = null; // N+
	Double _taperRatioOpt = null; // N+ (oswald factor)
	Amount<Angle> _sweepQuarterChordEq = null; // N+
	Amount<Angle> _sweepHalfChordEq = null; // N+
	Double _spanStationKink = null; // N+*

	MyArray _chordsVsYEq = new MyArray(SI.METER); // N+*
	MyArray _chordsVsYActual = new MyArray(SI.METER); // N+*
	MyArray _eta = new MyArray(Unit.ONE); // N+*
	MyArray _yStationEq = new MyArray(SI.METER); // N+*
	MyArray _yStationActual = new MyArray(SI.METER); // N+*
	MyArray _xLEvsYActual = new MyArray(SI.METER); // N+*
	MyArray _xTEvsYActual = new MyArray(SI.METER); // N+*
	MyArray _ellChordVsY = new MyArray(SI.METER);// N+*

	Double _extensionLERootChordLinPanel = null; // N+
	Double _extensionTERootChordLinPanel = null; // N+

	/** Wing root chord twist relative to body x-axis */
	Amount<Angle> _twistKink = null; // N+*
	Amount<Angle> _twistTip = null; // N+*

	Amount<Angle> _dihedralInnerPanel = null; // N+*
	Amount<Angle> _dihedralOuterPanel = null; // N+*
	Amount<Angle> _dihedralMean = null; // N+
	MyArray _dihedral = new MyArray(SI.RADIAN); // N+*
	double deltaFactorDrag; // N+

	CalcHighLiftDevices higLiftCalculator; // N+
	
	Amount<Length> _span = null; // N+ 
	 ...
	 */
	Amount<Length> _semispan = null; // N+ 

	// All N+*
	Amount<Length> _semiSpanInnerPanel = null;  // _semi_spanKink
	Amount<Length> _semiSpanOuterPanel = null;
	Amount<Length> _chordRootEquivalentWing = null;
	Amount<Length> _meanGeometricChord = null;
	...
	protected void initializeAirfoils(LiftingSurface2Panels ls) {
	 ...
	 */
	protected void initializeAirfoils(AircraftEnum aircraftName, LiftingSurface2Panels ls) {
	...
	public LiftingSurface2Panels(String name, 
			String description, 
			Double x, 
			Double y, 
			Double z, 
	...
	public LiftingSurface2Panels(ComponentEnum type) {
...
//				}
	...
	public void setHigLiftCalculator(CalcHighLiftDevices higLiftCalculator) {
		this.higLiftCalculator = higLiftCalculator;
	}
