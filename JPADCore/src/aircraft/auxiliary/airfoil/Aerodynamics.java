...
public class Aerodynamics extends AuxiliaryComponentCalculator{
	...
	public Aerodynamics(Airfoil airf, AircraftEnum aircraftName, AirfoilStationEnum station) {
		...
		case IRON:
			switch (station) {
			case ROOT:
				Aerodynamics(airf, AirfoilEnum.IRONROOT); //TODO: It should be Ha5 airfoil
				break;
			case KINK:
				Aerodynamics(airf, AirfoilEnum.IRONKINK) ;
				break;
			case TIP:
				Aerodynamics(airf, AirfoilEnum.IRONTIP);
				break;
			}
	...
	private void Aerodynamics(Airfoil airf, AirfoilEnum airfoilName) {
		 switch (airfoilName) {
		 ...
		 case IRONROOT: 
			...
			_alphaZeroLift = Amount.valueOf(Math.toRadians(-3.21), SI.RADIAN); 
			_clAlpha = 6.2457; 
			_alphaStar = Amount.valueOf(Math.toRadians(7.81),SI.RADIAN); // end-of-linearity 
			_clStar = 1.2 ; 
			_alphaStall = Amount.valueOf(Math.toRadians(17.0),SI.RADIAN); 
			_clMax = 1.83;
