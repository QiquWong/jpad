	...
	public void calculateAllPerformance() {
		...
		if (_theAircraft != null) {
			...
			performanceManager = new PerformanceCalcManager(
					...
					_theAircraft.getPowerPlant().get_engineList().get(0).getT0().doubleValue(SI.NEWTON),
					_theAircraft.getPowerPlant().get_engineNumber().intValue(), 
					_theAircraft.getPowerPlant().get_engineList().get(0).getEngineType(), 
					_theAircraft.getPowerPlant().get_engineList().get(0).getBPR(), 
