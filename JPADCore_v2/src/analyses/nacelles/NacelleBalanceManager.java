...
import calculators.balance.NacelleBalanceCalc;
import configuration.enumerations.ComponentEnum;
import configuration.enumerations.MethodEnum;
import standaloneutils.customdata.CenterOfGravity;
import writers.JPADStaticWriteUtils;

public class NacelleBalanceManager {
	...
	public void calculateTotalCG(Aircraft theAircraft, Map<ComponentEnum, MethodEnum> methodsMapBalance) {
		...
		_totalCG.setLRForigin(
				...
				theAircraft.getNacelles().getNacellesList().get(0).getYApexConstructionAxes(), 
				theAircraft.getNacelles().getNacellesList().get(0).getZApexConstructionAxes()
				);
		_totalCG.setXLRFref(theAircraft.getNacelles().getNacellesList().get(0).getLength().times(0.4));
		...
		_totalCG.calculateCGinBRF(ComponentEnum.NACELLE);
	}
	...
	private void calculateCG (Aircraft theAircraft, Map<ComponentEnum, MethodEnum> methodsMapBalance) {
		...
		if(!methodsMapBalance.get(ComponentEnum.NACELLE).equals(MethodEnum.AVERAGE)) {
			...
			_xCGEstimatedList.add(_xCGMap.get(methodsMapBalance.get(ComponentEnum.NACELLE)));
		}
		else {
			_percentDifference = new double[_xCGMap.size()];
	...
	private void calculateCG (Aircraft theAircraft, MethodEnum method) {
		...
		cg.setXLRFref(theAircraft.getNacelles().getNacellesList().get(0).getLength().times(0.4));
		...
		case TORENBEEK_1976:
			...
			cg.setXLRF(NacelleBalanceCalc.calculateNacelleXCGTorenbeek1976(theAircraft));
		...
		cg.calculateCGinBRF(ComponentEnum.NACELLE);
		_cgList.add(cg);
